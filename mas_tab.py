...
def on_submit():
    ...
    sumquery = ("SELECT SUM(net_amt) FROM rms_pos_txn_mas "
                f"WHERE sbu_code='{sbu_code}' AND loc_code='{location}' AND txn_date='{date}'")
    ...
    total_sum_query = ("SELECT SUM(net_amt) FROM rms_pos_txn_mas "
    ...
    rows, columns = execute_query(sbu_code, location, machine_no_list, date, receipt_no_list, status)
    display_results(rows, columns)

    # Execute the sum query
    sum_rows, sum_columns = execute_raw_query(sumquery, location)
    if sum_rows:
    ...
    total_sum_rows, total_sum_columns = execute_raw_query(total_sum_query, location)
    if total_sum_rows:
...
def display_results(rows, columns):
    ...
    if rows:
        ...
        treeview["show"] = "headings"  # Hide the default column
...
def create_mas_tab(notebook):
    mas_tab = ttk.Frame(notebook)
    notebook.add(mas_tab, text="MAS")
    
    form_frame = ttk.Labelframe(mas_tab, text="Server එකේ Mas Table එක", padding=(20, 10))
    ...
    global connection_message_label, row_count_label_value
    ...
    row_count_label = ttk.Label(top_right_frame, text="Row Count: ")
    row_count_label.grid(row=3, column=0, padx=5, pady=5, sticky=W)
    row_count_label_value = ttk.Label(top_right_frame, text="")
    row_count_label_value.grid(row=3, column=1, padx=5, pady=5, sticky=W)

    raw_query_frame = ttk.Labelframe(mas_tab, text="Raw SQL Query", padding=(20, 10))
    ...
    results_frame = ttk.Labelframe(mas_tab, text="Results", padding=(20, 10))
