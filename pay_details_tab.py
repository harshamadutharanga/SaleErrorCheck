...
from app_logic import process_form_data_pay, reset_form_fields_pay, execute_query_pay, execute_raw_query_pay
...
def on_submit(pay_details_tab):
    ...
    sumquery = ("SELECT SUM(amount) FROM rms_pos_pay_details "
                f"WHERE sbu_code='{sbu_code}' AND loc_code='{location}' AND txn_date='{date}'")
    ...
    total_sum_query = ("SELECT SUM(amount) FROM rms_pos_pay_details "
                      f"WHERE sbu_code='{sbu_code}' AND loc_code='{location}' AND txn_date='{date}'")
    
    if machine_no_list:
        total_sum_query += " AND (" + " OR ".join(machine_conditions) + ")"
    ...
    rows, columns = execute_query_pay(sbu_code, location, machine_no_list, date, receipt_no_list, paymode)
    display_results(rows, columns)

    # Execute the sum query
    sum_rows, sum_columns = execute_raw_query_pay(sumquery, location)
    if sum_rows:
    ...
    total_sum_rows, total_sum_columns = execute_raw_query_pay(total_sum_query, location)
    if total_sum_rows:
    ...
    if paymode:
        paymode_sum_query = ("SELECT SUM(amount) FROM rms_pos_pay_details "
                             f"WHERE sbu_code='{sbu_code}' AND loc_code='{location}' AND txn_date='{date}'"
        ...
        if machine_no_list:
            paymode_sum_query += " AND (" + " OR ".join(machine_conditions) + ")"
        
        paymode_sum_rows, paymode_sum_columns = execute_raw_query_pay(paymode_sum_query, location)
        if paymode_sum_rows:
...
def on_execute_raw_query_pay():
    ...
    if raw_query:
        rows, columns = execute_raw_query_pay(raw_query, location)
        display_results(rows, columns)
...
def create_pay_details_tab(notebook):
    pay_details_tab = ttk.Frame(notebook)
    notebook.add(pay_details_tab, text="PAY")
    
    form_frame_pay = ttk.Labelframe(pay_details_tab, text="Server එකේ Payment Details Table එක", padding=(20, 10))
    ...
    global entry_receipt_no_pay, paymode_combobox_pay, label_sum_value_pay, label_total_sum_value_pay
    global treeview_pay, entry_raw_query_pay, paymode_labels
    global connection_message_label, row_count_label_pay, row_count_label_pay_value
    ...
    button_submit_pay = ttk.Button(form_frame_pay, text="Submit", bootstyle="success", command=lambda: on_submit(pay_details_tab))
    ...
    raw_query_frame_pay = ttk.Labelframe(pay_details_tab, text="Raw SQL Query", padding=(20, 10))
    ...
    results_frame_pay = ttk.Labelframe(pay_details_tab, text="Results", padding=(20, 10))
